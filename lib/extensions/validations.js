 ...
 * @param {Object} metaObj is definition of property validations and sanitizers e.g. name:{ isString:true }
 * @param {Object} value property value
 * @returns {Object}  value and validation errors
 */
function validateProperty(schemaObj, value) {
    var output = {
        value: value,
        errs: []
    };
    ...
    for(var propName in schemaObj) {
        ...
        // schema property recognized as nested model
            ...
            if(SubModel.__typeof !== 'Model') {
                ...
                value = SubModel.new(value).validate();
                if(!value.isValid()) output.errs.push( value.validErrs() );
            }
        }
        ...
        // schema property recognized as nested value validations
            ...
            if(Object.prototype.toString.call(value) !== '[object Object]') output.errs.push( propName );
                ...
                for(var valueProp in value){
                    subOut = validateProperty(schemaObj[propName], value[valueProp]);
    ...
    instance: {
        ...
        validate: function(extendSchema){
            ...
            for(var propName in schema) {
                if(schema[propName].required === true || model.hasOwnProperty(propName)){
                    validResult = validateProperty(schema[propName], model[propName]);
        ...
        checkProp: function(propName, value){
            if(arguments.length===1) {
                return validateProperty(this.getSchema()[propName], this[propName]);
            }
            else {
                return validateProperty(this.getSchema()[propName], value);
            }
        },
...
Model.schemaProps = {
     ...
     * Extended sanitizers
     */
    setValue: sanitizer(validator.setValue), // always sets value
    defaultValue: sanitizer(validator.defaultValue), // sets value if null or undefined, or empty string
    sanitize: sanitizer(validator.customSanitization), // custom sanitizer function
    cleanUrl: sanitizer(validator.cleanUrl), // replace diacritics and other non "a-z0-9", "-", "_", "." characters
    ...
    isEmail: validator.isEmail, // (str) - check if the string is an email.
    isURL: validator.isUrl, // (str [, options]) - check if the string is an URL. options is an object which defaults to { protocols: ['http','https','ftp'], require_tld: true, require_protocol: false, allow_underscores: false, host_whitelist: false, host_blacklist: false }.
    isFQDN: validator.isFQDN, // (str [, options]) - check if the string is a fully qualified domain name (e.g. domain.com). options is an object which defaults to { require_tld: true, allow_underscores: false }.
     ...
     */
    toString: sanitizer(validator.toString), // (input) - convert the input to a string.
    toFloat: sanitizer(validator.toFloat), // (input) - convert the input to a float, or NaN if the input is not a float.
    toInt: sanitizer(validator.toInt), // (input [, radix]) - convert the input to an integer, or NaN if the input is not an integer.
    toInteger: sanitizer(validator.toInt), // alias for toInt
    toBoolean: sanitizer(validator.toBoolean), // (input [, strict]) - convert the input to a boolean. Everything except for '0', 'false' and '' returns true. In strict mode only '1' and 'true' return true.
    trim: sanitizer(validator.trim), // (input [, chars]) - trim characters (whitespace by default) from both sides of the input.
    ltrim: sanitizer(validator.ltrim), // (input [, chars]) - trim characters from the left-side of the input.
    rtrim: sanitizer(validator.rtrim), // (input [, chars]) - trim characters from the right-side of the input.
    escape: sanitizer(validator.escape), // (input) - replace <, >, &, ' and " with HTML entities.
    stripLow: sanitizer(validator.stripLow), // (input [, keep_new_lines]) - remove characters with a numerical value < 32 and 127, mostly control characters. If keep_new_lines is true, newline characters are preserved (\n and \r, hex 0xA and 0xD). Unicode-safe in JavaScript.
    whitelist: sanitizer(validator.whitelist), // (input, chars) - remove characters that do not appear in the whitelist.
    blacklist: sanitizer(validator.blacklist), // (input, chars) - remove characters that appear in the blacklist.
    normalizeEmail: sanitizer(validator.normalizeEmail), //(email) - canonicalize a gmail address.
};


// simple helper fro wrapping default validator sanitize methods
function sanitizer(fnc){
