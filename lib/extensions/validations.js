 ...
 */
function validateProperty(model, value, schemaObj) {
    var output = {
        value: value,
        model: model,
        errs: []
    ...
    for(var propName in schemaObj) {
        ...
        // schema property recognized as array of nested models
            ...
            else if(!SubModel.__typeof || !SubModel.__typeof('Model')) {
                throw new Error('Model validations: cannot validate, value by schema property "' +propName+ '" ' +
                                '- argument is not Model constructor');
            }
            else {
        ...
        // schema property recognized as nested value validations
            ...
            function iterator(valueProp){
                subOut = validateProperty(model, value[valueProp], schemaObj[propName]);
            ...
            if(Array.isArray(value)){
                ...
                for(var valueProp in value){
                    if(value.hasOwnProperty(valueProp)) iterator(valueProp);
                }
                if(subErrs) output.errs.push( subErrs );
            }
            else output.errs.push( propName );
        }
    ...
    for(var subProp in schemaProp) {
        if(Model.schemaProps[subProp] === 'nested_model') {
            ...
            for(var objSubProp in objProp)
                modelProp[objSubProp] = fillSubModels(schemaProp[subProp], objProp[objSubProp], skipFillHooks);
        }
    }
    return modelProp;
}
...
Model.extensions.push({
    instance: {
        ...
        validate: function(extendSchema){
            ...
            for(var propName in schema) {
                if(execValidation(schema[propName]) || model.hasOwnProperty(propName)){
                    validResult = validateProperty(model, model[propName], schema[propName]);
        ...
        checkProp: function(propName, value){
            if(arguments.length===1) {
                return validateProperty(this, this[propName], this.getSchema()[propName]);
            }
            else {
                return validateProperty(this, value, this.getSchema()[propName]);
            }
        },
         ...
         * @returns {Object}  model
         */
        fill: function(obj, extendSchema, skipFillHooks){
            obj = obj || {};
            ...
            for(var propName in obj) {
                if(schema[propName]) {
                    ...
                    if(schema[propName].containsNestedModel)
                        model[propName] = fillSubModels(schema[propName], obj[propName], skipFillHooks);
                    else
            ...
            if(!skipFillHooks) for(var i=0;i<onFillFncs.length;i++){
                modifiedModel = onFillFncs[i].call(model, model, skipFillHooks);
                if(modifiedModel) model = modifiedModel; // fallback if there is no return in onFill
            }
        ...
        fetch: function(obj){
            ...
            for(var i=0;i<onFetchFncs.length;i++){
                obj = onFetchFncs[i].call(model, obj) || obj;
            }
            
            return model.fill(obj, null, true);
        },
    ...
    constructor: function(ParentCnst){
        ...
        newConstructor._instanceListeners = newConstructor._instanceListeners || {};
        ...
        newConstructor.new = function(data, extendSchema, skipFillHooks){
            if(data) return (new this()).fill(data, extendSchema, skipFillHooks);
...
Model.schemaProps = {
    ...
    parseBuffer: validator.parseBuffer, // (value, encoding) check if value is Buffer, if not, try to create new Buffer
    toBuffer: validator.parseBuffer, // alias for parseDate
    buffer: validator.parseBuffer, // alias for parseDate
    validate: validator.customValidation, // custom validation function
    isPositiveNumber: validator.isPositiveNumber,
    isNegativeNumber: validator.isNegativeNumber,
    isNonPositiveNumber: validator.isNonPositiveNumber,
    isNonNegativeNumber: validator.isNonNegativeNumber,
    isGreaterThan: validator.isGreaterThan,
    isGreaterOrEqualTo: validator.isGreaterOrEqualTo,
    isLowerThan: validator.isLowerThan,
    isLowerOrEqualTo: validator.isLowerOrEqualTo,
    ...
    isUrl: validatorWrap(validator.isURL), // alias for isURL
