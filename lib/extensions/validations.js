    ...
    for(var subProp in schemaProp) {
        if(Model.schemaProps[subProp] === 'nested_model') {
            ...
            for(var objSubProp in objProp)
                modelProp[objSubProp] = fillSubModels(schemaProp[subProp], objProp[objSubProp], fillFromDataSource);
        }
    }
    return modelProp;
}
...
Model.extensions.push({
    instance: {
         ...
         * @returns {Object}  model
         */
        fill: function(obj, extendSchema, fillFromDataSource){
            obj = obj || {};
            ...
            for(var propName in obj) {
                if(schema[propName]) {
                    ...
                    if(schema[propName].containsNestedModel)
                        model[propName] = fillSubModels(schema[propName], obj[propName], fillFromDataSource);
                    else
            ...
            for(var i=0;i<onFillFncs.length;i++){
                modifiedModel = onFillFncs[i].call(model, model, fillFromDataSource);
    ...
    constructor: function(ParentCnst){
        ...
        newConstructor.new = function(data, extendSchema, fillFromDataSource){
            if(data) return (new this()).fill(data, extendSchema, fillFromDataSource);
