...
Model.extensions.push({
    ...
    constructor: function(ParentCnst){
        ...
        newConstructor.Collection.prototype.extendDefaults = function(defaults){
            object.extend('data', this._defaults, defaults);
            return this;
        };
        newConstructor.Collection.prototype.setDefaults = function(defaults, defaultsExtension){
            this._defaults = object.extend(true, {}, defaults, defaultsExtension || {});
        };
        ...
        newConstructor.collection = function(defaults){
            ...
            c.setDefaults(this._defaults, defaults);
            return c;
        };
...
function getOnlyData(records){
    if(Array.isArray(records)){
        for(var i=0;i<records.length;i++) records[i] = (records[i].__instanceof && records[i].__instanceof('Model')) ? records[i].getData() : records[i];
    }
    else records = (records && (records.__instanceof && records.__instanceof('Model'))) ? records.getData() : records;
