...
Cache.prototype.put = function(key, value, timeout, cb){ // cb(err, value)
    ...
    if(typeof timeout === 'string'){
        var timeType = timeout[ timeout.length-1 ];
        timeout = parseInt(timeout, 10);
        if(timeType==='s') timeout = timeout*1000;
        else if(timeType==='m') timeout = timeout*60*1000;
        else if(timeType==='h') timeout = timeout*60*60*1000;
        
        if(typeof timeout !== 'number') throw new Error('cache: duration is not in format "60s" or "10m" or "1h"');
    }
