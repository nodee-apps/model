...
ElasticSearch.extendDefaults({
    connection:{
        ...
        // version:'', elastic search engine version, loaded in init phase
        // versionNum: 0, version converted into number
        ...
        count:{
            ...
            url:'/_count',
            countKey:'count',
            transformRequest: function(reqOpts){
                delete reqOpts.query.sort;
                delete reqOpts.query._source;
                delete reqOpts.query.filter;
                delete reqOpts.query.post_filter;
                switchQueryAndBody.call(this, reqOpts);
            }
        },
        create:{
            method:'PUT',
            ...
            resourceKey:'this',
            idKey: '_id',
            transformRequest: removeDefaultQueryOptions
        },
        update:{
            transformRequest: function(reqOpts){
                delete reqOpts.query.sort;
                delete reqOpts.query.filter;
                delete reqOpts.query.post_filter;
                repairReqByVersion.call(this, reqOpts);
            }
        },
        remove:{
            transformRequest: function(reqOpts){
    ...
    options:{
        ...
        simulateOptimisticLock: true // will get old version and compare modifiedDT before update or remove
    },
    cache:{
...
function removeDefaultQueryOptions(reqOpts){
    delete reqOpts.query.sort;
    delete reqOpts.query._source;
    delete reqOpts.query.filter;
}

function repairReqByVersion(reqOpts){
    var ModelCnst = this;
    var versionNum = ModelCnst.getDefaults().connection.versionNum;

    // changes list: https://www.elastic.co/guide/en/elasticsearch/reference/current/breaking_50_search_changes.html
    if(versionNum > 5){

        if(reqOpts.query.search_type === 'count') {
            delete reqOpts.query.search_type;
            reqOpts.query.size = 0;
        }
        else if(reqOpts.query.search_type === 'scan') console.warn('search_type="scan" is removed in elasticsearch version > 5');

        if(reqOpts.query.fields) {
            reqOpts.query.stored_fields = reqOpts.query.fields;
            delete reqOpts.query.fields;
        }

        if(reqOpts.query.filter) {
            reqOpts.query.post_filter = reqOpts.query.filter;
            delete reqOpts.query.filter;
        }
    }
}
...
ElasticSearch.addMethod('init', function(cb){
    ...
    (function checkVersion(){
        request
        .get(defaults.connection.baseUrl)
        .end(function(err, res){

            var ver = (((res||{}).body||{}).version||{}).number || '0.0.0';

            ModelCnst.extendDefaults({
                connection:{
                    version: ver,
                    versionNum: parseFloat(ver.replace(/\./,',').replace(/\./g, '').replace(/,/,'.'), 10)
                }
            });
