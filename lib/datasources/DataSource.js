...
DataSource.Collection.addMethod(['sort','order'], function(sort){
    if(sort){
        for(var key in sort){
            if(sort[key]==='asc' || sort[key] === 1 || sort[key] === '1') sort[key] = 1;
...
DataSource.Collection.addMethod('cache', function(duration){
    ...
    this.extendDefaults({cache:{duration:duration, use:true}});
    return this;
});
...
DataSource.wrapHooks('create', function(callback){ // callback(err, doc)
    ...
    if(!model.isValid()) {
        ...
        model.constructor.collection().create(model.getData(), callback);
    }
});
...
DataSource.wrapHooks('update', function(callback){ // callback(err, doc)
    ...
    if(!model.isValid()) {
        ...
        model.constructor.collection().find(query).update(model.getData(), function(err, count){
            if(err) callback(err);
