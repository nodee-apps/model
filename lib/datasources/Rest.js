...
// on request before request send
Rest.addMethod('onRequest', function(request, defaults){
    // do something to change request
    // no callback, this is sync
});
...
function execRequest(ModelCnst, parseMode, defaults, reqData, done){ // done(err, data, count)
    ...
    function eReq(defaults, requestRepeats){
        ...
        ModelCnst.onRequest(request, defaults, reqData);
...
function readAll(ModelCnst, defaults, done){ // done(err, data, count)
    ...
    function readPage(defaults, allItems, requestRepeats){
        ...
        ModelCnst.onRequest(request, defaults);
        ...
        .end(function(err, res){
            if(err){
                ...
                else if(!dynamicPageSize && maxLimit && allItems.length >= maxLimit) done(null, allItems, count); // all items count >= maxLimit (we have enought items)
                else if(!dynamicPageSize && prevPageSize && prevPageSize > items.length) done(null, allItems, count); // page size < prev. page size ,page size is constant (there are no items left)
                else if(count === 0 || count <= readItemsCount) done(null, allItems, count); // read items = all items
                else if(requestedLimit && requestedLimit <= readItemsCount) done(null, allItems, count); // requested limit = readItemsCount (we have enought items)
