...
Rest.addMethod('buildRequest', function(defaults, reqData){
    ...
    if(typeof transformRequest === 'function') transformRequest(reqOpts);
    
    if(defaults.connection.debugRequest === true) {
        console.warn('\nRestDataSource request: ' + reqOpts.method +' '+ reqOpts.url);
        console.warn('RestDataSource request headers: ' + JSON.stringify(reqOpts.headers, null, 4));
        console.warn('RestDataSource request query: ' + JSON.stringify(reqOpts.query, null, 4));
        console.warn('RestDataSource request body: ' + JSON.stringify(reqOpts.body, null, 4));
    }
...
Rest.addMethod('authRequest', function(ModelCnst, defaults, reqData, done){ // done(err)
...
function execRequest(ModelCnst, defaults, reqData, done){ // done(err, data, count, rawResponse)
    ...
    var allItems = [],
        ...
        prevPageSize = 0,
        isList = (defaults.connection[ defaults.connection.command ]||{}).isList,
        maxRetries = defaults.options.maxRetries;

    defaults.options.skip = defaults.options.skip || 0;
        
    if(ModelCnst.authRequest) ModelCnst.authRequest(ModelCnst, defaults, reqData, function(err){
    ...
    function eReq(defaults){
        ...
        .end(function(err, res){
            ...
            if(typeof defaults.connection.debugResponse === 'function') defaults.connection.debugResponse(res);
                ...
                console.warn('\nRestDataSource response for: ' + request.method +' '+ request.url);
                console.warn('RestDataSource response statusCode: ' + res.status);
                console.warn('RestDataSource response headers: ' + JSON.stringify(res.headers, null, 4));
                console.warn('RestDataSource response body: ' + JSON.stringify(res.body || res.text, null, 4));
            }
            
            if(!res && err) done(new Error((ModelCnst._name||'RestDataSource')+': Request failed "' +request.url+ '"').cause(err));
            else if(!res && !err) done(new Error((ModelCnst._name||'RestDataSource')+': Request failed "' +request.url+ '"'));
            
            
            else if(res.statusType === 5){ // server error
                done(res.error, ModelCnst.parseError(defaults, res.status, res.body), null, res);
            }
                ...
                var data = ModelCnst.parseResource(defaults, res.status, res.body);
                var count = ModelCnst.parseCount(defaults, res.status, res.body);
