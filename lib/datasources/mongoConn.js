...
const promise = require('nodee-utils').async.Series.promise;
...
function createMongoUrl(opts){
    ...
    // host can be used as connection string - for backward compatibility
    if(!connString && opts.host.match(/^(mongodb|mongodb\+srv|):\/\//)) return opts.host;

    if(!connString && (!opts.host || !opts.database)) {
...
function getClient(mongoUrl, callback) { // callback(err, mongoClient)
    ...
    if(dbPromise) { // mongo client db for this connection already promised
        ...
        mongoDbs[mongoUrl] = promise(function(promise){
            MongoClient.connect(mongoUrl, { useNewUrlParser: true, useUnifiedTopology: true }, function(err, mongoClient){
