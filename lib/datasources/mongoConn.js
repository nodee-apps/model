...
function createMongoUrl(opts){
    ...
    if(opts.connString) {
        if(!getDatabaseDromConnString(opts.connString)) throw new Error('Model datasources mongoConnector: connection string must contains database name');
        return opts.connString;
    }
    
    // only connection prefs are copyied to mongoUrl
    var nonPrefs = ['connName', 'connString', 'host', 'port', 'database', 'collection', 'username', 'password', 'mongoUrl', 'indexes'];
