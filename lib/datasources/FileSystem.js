...
var Model = require('../model.js'),
    ...
    sift = require('enterprise-utils').sift;
...
var FileSystem = module.exports = Model.define('FileSystemDataSource',['DataSource', 'Tree'], {
    ...
    content:{ hidden:true }, // used when creating new file, do not send it back
    data:{ hidden:true } // alias for content
});
...
function getData(dirPath, findQuery, cb){
...
FileSystem.Collection.addMethod('create', { cacheable:false, fetch:true }, function(data, cb){ // cb(err, doc/docs)
    ...
    async.Series.each(data, function(i, next){
        fsExt.checkExistsName(data[i].fullPath, data[i].isDir, function(err, fullPath){
            if(err) next(err);
            else {
                var oldFileName = data[i].fullPath.split('/').pop();
                var newFileName = fullPath.split('/').pop();
                data[i].id = data[i].id.replace(new RegExp(oldFileName.escape() + '$'), newFileName);
                
                if(data[i].isFile) {
                    fsExt.writeFile(fullPath, (data[i].content || data[i].data || ''), function(err){
                        if(err) next(err);
                        else {
                            fsExt.getFileInfo(data[i].id, fullPath, function(err, info){
                                if(err) next(err);
                                else {
                                    data[i] = info;
                                    next();
                ...
                else fs.mkdir(fullPath, function(err){
                    if(err) next(err);
                    else {
                        fsExt.getFileInfo(data[i].id, fullPath, function(err, info){
                            if(err) next(err);
...
FileSystem.Collection.addMethod('update', { cacheable:false, fetch:false }, function(data, cb){ // cb(err, count)
    ...
    // files cannot be directly updated, use instance methods such as "write","rename"
    cb(null, 0);
});
