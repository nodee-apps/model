...
var Model = require('../model.js'),
    ...
    generateId = neUtils.shortId.generate;
...
function getFileData(files, opts, cb){
    ...
    if((singleFile && !files) || (!singleFile && !files.length)) return cb(null, files);
...
FileSystem.Collection.addMethod('exec', { cacheable:true, fetch:false }, function(command, args, cb){ // cb(err, count)
    ...
    getFileList(defaults.connection.dirPath, defaults.query, function(err, records){
        ...
        var matchedRecords = neUtils.query.filter(defaults.query, records);
...
FileSystem.Collection.addMethod('update', { cacheable:false, fetch:false }, function(data, cb){ // cb(err, count)
    ...
    getFileList(defaults.connection.dirPath, defaults.query, function(err, records){
        ...
        var toUpdate = neUtils.query.filter(defaults.query, records);
        var count = 0;
...
FileSystem.Collection.addMethod('remove', { cacheable:false }, function(cb){ // cb(err, count)
    ...
    getFileList(defaults.connection.dirPath, defaults.query, function(err, records){
        ...
        var toRemove = neUtils.query.filter(defaults.query, records);
        var count = 0;
        var removedFolders = [];
...
FileSystem.wrapHooks('write', function(data, opts, cb){ // cb(err, updatedFile)
    var file = this;
    if(arguments.length === 2){
        cb = arguments[1];
        opts = {};
    ...
    var optimisticLock = file.constructor.getDefaults().options.optimisticLock;
    if(opts.hasOwnProperty('optimisticLock')) optimisticLock = opts.optimisticLock;
    else if(opts.force) optimisticLock = false;
    
    if(!file.id) return cb(new Error((file.constructor._name||'FileSystemDataSource')+'.prototype.write: INVALID').details({ code:'INVALID', validErrs:{ id:['required'] } }));
    var fullPath = file.constructor.getDefaults().connection.dirPath + '/' + file.id;
