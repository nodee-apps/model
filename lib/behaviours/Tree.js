...
Tree.on('beforeRemove', 'removeChildren', function(next){
    ...
    doc.constructor.collection().find({ ancestors:doc.id }).remove(function(err){
        if(err) next(new Error((doc.constructor._name||'Tree')+' on beforeRemove: cannot remove descendants, ID "' +doc.id+ '"').cause(err));
        else next();
    });
});

// delete all descendants
Tree.on('afterRemove', 'updateParentRef', function(args, next){
    var doc = this;
    updateParent('removeRef').call(doc, next);
});
    ...
    return function(args, next){
        ...
        if(!doc.ancestors) next(new Error((doc.constructor._name||'Tree')+' updateParent: missing ancestors, ID "' +doc.id+ '"').details({ code:'INVALID', validErrs:{ ancestors:['required'] } }));
            ...
            doc.constructor.collection().findId(parentId).update(updateExp, function(err){
                if(err) next(new Error((doc.constructor._name||'Tree')+' updateParent: cannot update parent, ID "' +doc.id+ '"').cause(err));
                else next();
            });
        }
        else next();
...
Tree.prototype.validateAncestors = function(cb){ // cb(err)
    ...
    if(doc.ancestors && doc.ancestors.length>0){
        ...
        if(doc.id && doc.ancestors.indexOf(doc.id) > -1) return cb(new Error((doc.constructor._name||'Tree')+'.prototype.validateAncestors: Ancestor cannot be document itself "' +JSON.stringify(doc.ancestors)+ '", ID "' +doc.id+ '"').details({ code:'INVALID', validErrs:{ ancestors:['invalid'] } }));
        
        var uniqueAncIds = {};
        for(var i=0;i<doc.ancestors.length;i++){
            if(uniqueAncIds[ doc.ancestors[i] ]) return cb(new Error((doc.constructor._name||'Tree')+'.prototype.validateAncestors: Ancestor duplicities "' +JSON.stringify(doc.ancestors)+ '", ID "' +doc.id+ '"').details({ code:'INVALID', validErrs:{ ancestors:['invalid'] } }));
        ...
        doc.constructor.collection().findId(doc.ancestors).fields({ id:true, ancestors:true }).all(function(err, ancs){
            ...
            else if(ancs.length !== newPath.length) cb(new Error((doc.constructor._name||'Tree')+'.prototype.validateAncestors: cannot find all ancestors "' +JSON.stringify(doc.ancestors)+ '", ID "' +doc.id+ '"').details({ code:'EXECFAIL' }));
            else {
                    ...
                    for(var a=0;a<ancs.length;a++){
                        if(ancs[a].id === newPath[i]) {
                            if(parentPathLength > -1 && parentPathLength + 1 !== ancs[a].ancestors.length) {
                                return cb(new Error((doc.constructor._name||'Tree')+'.prototype.validateAncestors: inconsistent tree path "' +JSON.stringify(doc.ancestors)+ '", ID "' +doc.id+ '"'));
                            }
...
Tree.prototype.move = Tree.wrapHooks('move', function(parentId, callback){ // callback(err, doc)
    ...
    if(doc.id===parentId) callback(new Error((doc.constructor._name||'Tree')+'.prototype.move: Parent is same as child, ID "' +doc.id+ '"').details({ code:'INVALID', validErrs:{ ancestors:['invalid'] } }));
    ...
    // dont trust user data, load old document to ensure data integrity
        ...
        if(err) callback(err);
        else if(!oldDoc) callback(new Error((doc.constructor._name||'Tree')+'.prototype.move: Document not found, ID "' +doc.id+ '"').details({ code:'NOTFOUND' }));
            ...
            doc.constructor.collection().findId(doc.id).update(updSet, function(err, updated){
                if(err) callback(err);
                else if(updated!==1) callback(new Error((doc.constructor._name||'Tree')+'.prototype.move: Updating failed, document not found, ID "' +doc.id+ '"').details({ code:'NOTFOUND' }));
            ...
            doc.constructor.collection().findId(parentId).one(function(err, parent){
                if(err) callback(err);
                else if(!parent) callback(new Error((doc.constructor._name||'Tree')+'.prototype.move: Parent not found, ID "' +doc.id+ '"').details({ code:'INVALID', validErrs:{ ancestors:['invalid'] } }));
                else {
                    // update document ancestors
    ...
    function updateDescendants(doc, oldDoc, callback){
        doc.constructor.collection().find({ ancestors:doc.id }).fields({ ancestors:1 }).all(function(err, descendants){
            if(err) callback(new Error((doc.constructor._name||'Tree')+'.prototype.move: failed to get descendants, ID "' +doc.id+ '"').cause(err));
                ...
                async.Series.each(descendants, function(i, next){
                    ...
                    doc.constructor.collection().findId(descendants[i].id).update(updSet, function(err, desc){
                        if(err) next(new Error((doc.constructor._name||'Tree')+'.prototype.move: failed to update descendant, ID "' +doc.id+ '"').cause(err));
                        else next();
                    });
                }, function(err){
                    if(err) callback(err);
